...
--7. Liste des trophées : numéro, date de prise, nom de la catégorie et nom du preneur. (10lignes)
 ...
 from trophee t  join categorie c on t.code_cat = c.code_cat
 join habitant h on h.num_hab = t.num_preneur ;

--8. Nom des habitants qui habitent à Aquilona. (7 lignes)
select h.nom
from habitant h join village v ON v.num_village = h.num_village where v.nom_village  = 'Aquilona';

--9. Nom des habitants ayant pris des trophées de catégorie Bouclier de Légat. (2 lignes)
select h.nom
from habitant h join trophee t on h.num_hab = t.num_preneur 
...
from potion p join fabriquer f on p.num_potion = f.num_potion 
...
join habitant h2 on f.num_hab =h2.num_hab where h2.nom = 'Amnésix';

--14. Nom des habitants dont la qualité n'est pas renseignée. (2 lignes)

select h.nom , h.num_qualite 
from habitant h where h.num_qualite isnull ;
...
select h.nom, h.age 
from habitant h order by h.nom ;
...
from habitant h join trophee t on h.num_hab = t.num_preneur group by h.nom ;

--25. Moyenne d'âge des habitants par province (nom de province, calcul). (3 lignes)
select  p.nom_province , round (avg  (h.age) )
from province p join village v on p.num_province = v.num_province 
join habitant h on v.num_village = h.num_village group by p.nom_province ;
--26. Nombre de potions différentes absorbées par chaque habitant (nom et nombre). (9lignes)

select h.nom, count (a.quantite)
from habitant h join absorber a on h.num_hab = a.num_hab group by h.nom ;
...
from village v join resserre r on v.num_village = r.num_village
--29. Nom du village contenant le plus grand nombre de huttes. (Gergovie)

select  v.nom_village
from village v where v.nb_huttes  = (select max (v2.nb_huttes) from village v2 )
